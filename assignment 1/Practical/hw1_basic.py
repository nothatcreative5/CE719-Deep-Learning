...
def count_tensor_elements(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  num_elements = torch.prod(torch.tensor(x.shape))
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return num_elements
...
def zero_row_min(x):
  ...
  # Replace "pass" statement with your code
  y = x.clone()
  _, row_index = torch.min(x, dim = 1)
  y[torch.arange(0, x.shape[0]), row_index] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def normalize_columns(x):
  ...
  column_mean = x.sum(dim = 0) / x.shape[0]
  centered = x - column_mean
  denom = (x.shape[0] - 1)
  column_std = torch.sqrt(torch.sum(centered ** 2, dim = 0) /denom)

  y = (x - column_mean) / column_std
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
